...
do_filter_flags() {
	...
	#export CFLAGS="${CFLAGS} -Wno-return-mismatch -Wno-incompatible-pointer-types -Wno-implicit-function-declaration -Wno-implicit-int -Wno-int-conversion -Wno-declaration-missing-parameter-type"
}
...
src_prepare() {
	...
	#sed -i 's/^      ${CC-cc} $dummy.c/      ${CC-cc} ${CFLAGS} $dummy.c/' config.if
